...
dataset$PRIORFRAC<- as.factor(dataset$PRIORFRAC)
dataset$MOMFRAC<- as.factor(dataset$MOMFRAC)
dataset$ARMASSIST<- as.factor(dataset$ARMASSIST)
dataset$SMOKE<- as.factor(dataset$SMOKE)
dataset$RATERISK<- as.factor(dataset$RATERISK)
dataset$FRACTURE<- as.factor(dataset$FRACTURE)
dataset$PREMENO<- as.factor(dataset$PREMENO)
...
```{r}
library(ROSE)
over <- ovun.sample(FRACTURE~.,data=validationData,method="over", N=200)$data
table(over$FRACTURE)
